/* 1. Promise 是一个 构造函数，可以  new Promise() 得到一个 Promise 的实例；
 * 2. 在 Promise上，有两个函数，分别叫做 resolve（成功之后的回调函数） 和 reject（失败之后的回调函数）
 * 3. 在 Promise构造函数的 Prototype 属性上，有一个 .then() 方法，
 * 也就说，只要是 Promise 构造函数创建的实例，都可以访问到 .then() 方法
 * 4. Promise 表示一个 异步操作；每当我们 new 一个 Promise 的实例，这个实例就表示一个具体的异步操作；
 * 5. 既然 Promise 创建的实例，是一个异步操作，那么这个 异步操作的结果，只能有两种状态：
 *  5.1 状态1： 异步执行成功了，需要在内部调用 成功的回调函数 resolve 把结果返回给调用者；
 *  5.2 状态2： 异步执行失败了，需要在内部调用 失败的回调函数 reject 把结果返回给调用者；
 *  5.3 由于 Promise 的实例，是一个异步操作，所以，内部拿到 操作的结果后，
 *  无法使用 return 把操作的结果返回给调用者； 这时只能使用回调函数的形式，把成功或失败的结果返回给调用者；
 ...
 * 也就是说，new 的时候，除了能够得到 一个 promise 实例之外，
 * 还会立即调用 我们为 Promise 构造函数传递的那个 function，执行这个 function 中的 异步操作代码；*/
/*var promise = new Promise(function(){
